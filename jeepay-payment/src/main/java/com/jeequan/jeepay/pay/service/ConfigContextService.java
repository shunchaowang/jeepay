...
public class ConfigContextService {
    ...
    if (mchInfo.getType() == CS.MCH_TYPE_NORMAL) {
      ...
      for (PayInterfaceConfig payInterfaceConfig : allConfigList) {
        mchAppConfigContext.getNormalMchParamsMap().put(
            ...
            NormalMchParams.factory(payInterfaceConfig.getIfCode(),
      ...
      AlipayNormalMchParams alipayParams = mchAppConfigContext.getNormalMchParamsByIfCode(
      ...
      //放置 wxJavaService
      WxpayNormalMchParams wxpayParams = mchAppConfigContext.getNormalMchParamsByIfCode(
      ...
      //放置 paypal client
      PppayNormalMchParams ppPayMchParams = mchAppConfigContext.getNormalMchParamsByIfCode(
      ...
      // set up ems wrapper
      EmspayNormalMchParams emspayNormalMchParams = mchAppConfigContext.getNormalMchParamsByIfCode(
          IF_CODE.EMSPAY, EmspayNormalMchParams.class);
      if (emspayNormalMchParams != null) {
        mchAppConfigContext.setEmsWrapper(EmsWrapper.buildEmsWrapper(emspayNormalMchParams));
      }
      ...
      for (PayInterfaceConfig payInterfaceConfig : allConfigList) {
        mchAppConfigContext.getIsvsubMchParamsMap().put(
            payInterfaceConfig.getIfCode(),
            IsvsubMchParams.factory(payInterfaceConfig.getIfCode(),
                payInterfaceConfig.getIfParams())
        );
      }
  ...
  public synchronized void initIsvConfigContext(String isvNo) {
    ...
    mchInfoService.list(MchInfo.gw().select(MchInfo::getMchNo).eq(MchInfo::getIsvNo, isvNo))
        .forEach(r -> mchNoList.add(r.getMchNo()));

    // 查询出所有 所属当前服务商的所有应用集合
    List<String> mchAppIdList = new ArrayList<>();
    if (!mchNoList.isEmpty()) {
      mchAppService.list(MchApp.gw().select(MchApp::getAppId).in(MchApp::getMchNo, mchNoList))
          .forEach(r -> mchAppIdList.add(r.getAppId()));
    }
    ...
    //放置alipay client
    AlipayIsvParams alipayParams = isvConfigContext.getIsvParamsByIfCode(CS.IF_CODE.ALIPAY,
        AlipayIsvParams.class);
    if (alipayParams != null) {
    ...
    //放置 wxJavaService
    WxpayIsvParams wxpayParams = isvConfigContext.getIsvParamsByIfCode(CS.IF_CODE.WXPAY,
        WxpayIsvParams.class);
    if (wxpayParams != null) {
